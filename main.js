...
let projects = [
  {
    image: 'textures/project1gunsmain.png',
    url: 'https://guns.lol/tranquil',
  },
  {
    image: 'textures/quizzie.png',
    url: 'https://101176.stu.sd-lab.nl/Quiz/',
  },
  {
    image: 'textures/2d-porto.png',
    url: 'https://101176.stu.sd-lab.nl/port/',
  },
  {
    image: 'textures/love.png',
    url: 'https://dijknoel.github.io/noxgift.github.io/',
  }
...
gltfLoader.load(
  'models/fullroom-furn.glb',
  function (room) {
    // Hide loader on load
    loaderWrapper.style.display = 'none';

    // Remove any unwanted cube geometry if present
    room.scene.traverse((child) => {
      if (child.name === "Cube" || child.name === "Room_Cube") {
        child.visible = false;
      }
    });
    ...
    video.src = 'textures/background-1.mp4';
    video.muted = true;
...
const roomLight = new THREE.PointLight(0xff9966, 1.5, 10);
...
const fanLight1 = new THREE.PointLight(0xff5f1f, 0.5, 0.5);
const fanLight2 = new THREE.PointLight(0xff5f1f, 0.5, 0.5);
const fanLight3 = new THREE.PointLight(0xff5f1f, 0.5, 0.5);

// Position lights on the PC fans
fanLight1.position.set(-0.3, 0.5, -0.3);
fanLight2.position.set(-0.3, 0.3, -0.3);
fanLight3.position.set(-0.3, 0.1, -0.3);
...
function loadIntroText() {
  ...
  loader.load('fonts/unione.json', function (font) {
    ...
    const titleGeo = new TextGeometry('Silent', {
      font: font,
      size: 0.08,
      height: 0.01,
    });
    ...
    titleText.position.set(-0.27, 0.65, 0.43);
    scene.add(titleText);
  });
  ...
  loader.load('fonts/helvatica.json', function (font) {
    ...
    const subTitleGeo = new TextGeometry(
      'Developer / Web coder / Creative coder',
      {
        font: font,
        size: 0.018,
        height: 0,
    ...
    subtitleText.position.set(-0.255, 0.6, 0.5);
    scene.add(subtitleText);
  });
}

function switchTheme(themeType) {
  // Add smooth rotation to light switch
  gsap.to(lightSwitch.rotation, {
    z: themeType === 'dark' ? Math.PI / 7 : 0,
    duration: 1,
    ease: "power2.inOut"
  });
  ...
  if (themeType === 'dark') {
    ...
    // main lights with smoother transition
    ...
    // fan lights with smooth transition - orange glow
    gsap.to(fanLight1, {
      intensity: 0.8,
      duration: 1,
      ease: "power2.inOut"
    });
    gsap.to(fanLight1.color, {
      r: 1,      // Orange color
      g: 0.37,
      b: 0.12,
      duration: 1,
    ...
    // text transitions
    gsap.to([titleText.material[0].color, subtitleText.material[0].color], {
      r: 0.98,
      g: 0.4,
      b: 0.2,
      duration: 1,
    ...
    gsap.to([titleText.material[1].color, subtitleText.material[1].color], {
      r: 1,
      g: 0.9,
      b: 0.8,
      duration: 1,
    ...
    gsap.to([pointLight1, pointLight2, pointLight3, pointLight4], {
      intensity: 0.4,
      duration: 1,
      ease: "power2.inOut"
    });
    gsap.to([pointLight1.color, pointLight2.color, pointLight3.color, pointLight4.color], {
      r: 1,
      g: 0.42,
      b: 0.21,
      duration: 1,
    ...
    // Light theme - subtle orange glow
    ...
    gsap.to([fanLight1.color, fanLight2.color, fanLight3.color], {
      r: 1,      // Lighter orange
      g: 0.5,
      b: 0.2,
      duration: 1,
    ...
    gsap.to([fanLight1, fanLight2, fanLight3], {
      intensity: 0.3,
      duration: 1,
      ease: "power2.inOut"
    });
